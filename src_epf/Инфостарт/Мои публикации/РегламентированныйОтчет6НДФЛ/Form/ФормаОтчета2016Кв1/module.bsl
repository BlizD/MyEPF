...
Процедура ОбособленноеПодразделениеНачалоВыбораИзСписка(Элемент, СтандартнаяОбработка)
	...
	Если ЭлементыФормы.ОбособленноеПодразделение.СписокВыбора.Количество() > 0 Тогда
		Возврат
	КонецЕсли;
	...
	ИначеЕсли ОбособленноеПодразделение.Метаданные().Реквизиты.Найти("КодПоОКТМО") <> Неопределено Тогда
		...
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ПодразделенияОрганизаций.Наименование + "": "" + ПодразделенияОрганизаций.КодПоОКТМО + "", "" + ПодразделенияОрганизаций.КПП КАК Представление,
		|	ПодразделенияОрганизаций.Ссылка КАК Ссылка,
		...
		|	И (ПодразделенияОрганизаций.КодПоОКТМО <> &КодПоОКТМО_Орг
		|			ИЛИ ПодразделенияОрганизаций.КПП <> &КПП_Орг)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Представление";
		
		СтрокаСведений = "КППЮЛ, ОКТМО";
		СведенияОбОрганизации = РегламентированнаяОтчетность.ПолучитьСведенияОбОрганизации(Организация, ДатаПодписи, СтрокаСведений);
		ЗапросПоПодразделениям.УстановитьПараметр("ПустойКодПоОКТМО","");
		ЗапросПоПодразделениям.УстановитьПараметр("КодПоОКТМО_Орг", СведенияОбОрганизации.ОКТМО);
		ЗапросПоПодразделениям.УстановитьПараметр("КПП_Орг", СведенияОбОрганизации.КППЮЛ);
		ЗапросПоПодразделениям.УстановитьПараметр("Организация", Организация);
		Выборка = ЗапросПоПодразделениям.Выполнить().Выбрать();
...
Процедура ИнициализироватьНастройкиПечатиСтраниц()
	...
	СтрокаУровня1 = мДеревоСтраницОтчета.Строки.Добавить();
	СтрокаУровня1.ИмяСтраницы              = "Раздел1";
	СтрокаУровня1.Представление            = "Разделы 1 и 2";
...
Процедура ОчиститьОбособленноеПодразделение()
	...
	ЭлементыФормы.ОбособленноеПодразделение.СписокВыбора.Очистить();
...
Процедура Расчет(ИмяРаздела = "") Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИмяРаздела) ИЛИ ИмяРаздела = "Раздел2" Тогда
		...
		Пока ПТД_Раздел2.Области.Найти("П000020013001" + Постфикс) <> Неопределено Цикл
			Строка130 = ПТД_Раздел2.Области["П000020013001" + Постфикс].Значение;
			ИтогСтрока130 = ИтогСтрока130 + Строка130;
			
			Строка140 = ПТД_Раздел2.Области["П000020014001" + Постфикс].Значение;
			ИтогСтрока140 = ИтогСтрока140 + Строка140;
			
			Инкр(Инд);
			Постфикс = "_" + СтрокаЧГ0(Инд);
		КонецЦикла;
		
		ПТД_Раздел2.Области.П000020013002.Значение = ИтогСтрока130;
		ПТД_Раздел2.Области.П000020014002.Значение = ИтогСтрока140;
...
Процедура ОчиститьТабличноеПоле(ВыбТабличноеПоле) Экспорт
	...
	Для каждого Область Из ВыбТабличноеПоле.Области Цикл
		Если Область.ЦветФона = мСтруктураЦветовРаскраски._4 И Область.Защита Тогда
			Область.Защита = Ложь;
...
Процедура ПроверитьЗаполнение_Раздел2(Параметры, СтатусПроверки)
	...
	Пока ДанныеРаздела.Свойство("П000020010001_" + СтрокаЧГ0(Инд)) Цикл
		...
		Если СтрокаЗаполнена Тогда
			ИмяПоказателя = "П000020010001_" + НомерСтроки;
			Строка100 = ДанныеРаздела[ИмяПоказателя];
			
			Если ЗначениеЗаполнено(Строка100) И НЕ РегламентированнаяОтчетность.УдовлетворяетТребованиямНО5_ДатаТип(Строка100) Тогда
			...
			ИмяПоказателя = "П000020011001_" + НомерСтроки;
			Строка110 = ДанныеРаздела[ИмяПоказателя];
			
			Если ЗначениеЗаполнено(Строка110) И НЕ РегламентированнаяОтчетность.УдовлетворяетТребованиямНО5_ДатаТип(Строка110) Тогда
			...
			ИмяПоказателя = "П000020012001_" + НомерСтроки;
			Строка120 = ДанныеРаздела[ИмяПоказателя];
			
			Если ЗначениеЗаполнено(Строка120) И НЕ РегламентированнаяОтчетность.УдовлетворяетТребованиямНО5_ДатаТип(Строка120) Тогда
...
Процедура ЗафиксироватьОшибкуЗаполнения(СтатусПроверки, Описание, ИмяРаздела, ИмяПоказателя = "", НомерСтраницы = "")
	
	РегламентированнаяОтчетность.роСообщитьВТаблицуСообщений(ЭтаФорма, Описание, ИмяРаздела, ИмяПоказателя, НомерСтраницы);
	СтатусПроверки = Ложь;
	...
	Для Каждого Стр из Узел.Строки Цикл
		Если Стр.Тип = "С" ИЛИ Стр.Тип = "C" Тогда // учтем оба варианта: кириллицу и латиницу
			Если РегламентированнаяОтчетность.СвойствоОпределено(Параметры, "ИмяКлючевогоУзлаСодержательнойЧасти") И Стр.Код = Параметры.ИмяКлючевогоУзлаСодержательнойЧасти Тогда
				ЗаполнитьУзел_НДФЛ6(Параметры, Стр);
			Иначе
...
Процедура ЗаполнитьУзел_НДФЛ6(Параметры, УзелРодитель)
	...
	Пока РегламентированнаяОтчетность.СвойствоОпределено(Раздел2, "П000020010001_" + Формат(Инд, "ЧГ=0")) Цикл
		...
		Если СтрокаЗаполнена Тогда
			...
			УстановитьЗначениеЭлемента(Узел_СумДата, "ДатаФактДох",  Формат(Раздел2["П000020010001" + Постфикс], "ДФ=dd.MM.yyyy; ДП=00.00.0000"));
			УстановитьЗначениеЭлемента(Узел_СумДата, "ДатаУдержНал", Формат(Раздел2["П000020011001" + Постфикс], "ДФ=dd.MM.yyyy; ДП=00.00.0000"));
			УстановитьЗначениеЭлемента(Узел_СумДата, "СрокПрчслНал", Формат(Раздел2["П000020012001" + Постфикс], "ДФ=dd.MM.yyyy; ДП=00.00.0000"));
...
Процедура ИнициализироватьСтруктурыДанных()
	...
	мСтруктураЦветовРаскраски = РегламентированнаяОтчетность.ЦветаРаскраскиПоказателей();
...
Функция ВыполняетсяКС_1_1(ОбработкаКС, СтруктураПараметров)
	...
	Для Каждого Стр из ТаблицаПодходящихДокументов Цикл
		...
		Если Титульный.НомерКорректировки <> "000" Тогда
